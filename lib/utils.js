...
(function() {
  var CLOUDINARY_JS_CONFIG_PARAMS, CONDITIONAL_OPERATORS, CONDITIONAL_PARAMETERS, DEFAULT_RESPONSIVE_WIDTH_TRANSFORMATION, LAYER_KEYWORD_PARAMS, _, build_custom_headers, build_eager, config, crc32, crypto, finalize_resource_type, finalize_source, generate_token, hashToQuery, join_pair, norm_range_value, number_pattern, offset_any_pattern, offset_any_pattern_re, parse_expression, process_if, process_layer, process_video_params, querystring, smart_escape, split_range, textStyle, unsigned_url_prefix, url, utf8_encode, utils, v1_adapter, v1_result_adapter,
    slice = [].slice;
  ...
  parse_expression = function(expression) {
    var replaceRE;
    if (!_.isString(expression) || expression.match(/^!.+!$/)) {
      return expression;
    }
  ...
  process_if = function(ifValue) {
    if (ifValue) {
      return "if_" + parse_expression(ifValue);
    } else {
      return ifValue;
    }
  };
